...
- Deployed to `claude-dev` (deploy id `0Affj00000RkQuPCAV`): 14 components, 2 Apex tests passed, 100% coverage on `TicketBoardController`. 6 Jest tests pass, ESLint and Prettier clean.
- `Ticket_Manager` assigned to the org user and four sample tickets (TKT-0000 to TKT-0003) created for manual checks.
- Check-only deploys (`sf project deploy validate`) fail the Apex tests with "fields being inaccessible on Sobject Ticket__c" because new fields do not exist during validation. Use a real deploy to run the tests.
- Apex tests seed data inside `System.runAs` of a user holding `Ticket_Manager`, because the deploying user has no CRUD on a newly created object while the deployment runs.
- A required field (`Status__c`) cannot be listed in permission set `fieldPermissions`; the platform rejects the deploy.
- Local Node is v14, but ESLint 9 and sfdx-lwc-jest 7 need Node 18+. Workaround: prepend `C:\Program Files\sf\client\bin` (the sf CLI's bundled Node 22) to PATH before `npm run lint` / `npm test`. Consider upgrading Node.
